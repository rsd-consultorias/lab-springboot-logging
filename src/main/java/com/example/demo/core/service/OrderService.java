...
public class OrderService {
    ...
    public ServiceResponse createOrderV2(CreateOrderDTO createOrderDTO) {
        createOrderDTO.setId(UUID.randomUUID());

        businessLogService.info("ORDER", "Order Creation", "%s".formatted(createOrderDTO.getId()),
                "Initializing order for CPF %s".formatted(createOrderDTO.getCpf()));

        businessLogService.info("ORDER", "Order Creation", createOrderDTO.getId().toString(),
                "Verifying if requested CPF %s is fraud".formatted(createOrderDTO.getCpf()));
        if (fraudService.isFraud(createOrderDTO.getCpf())) {
            businessLogService.warn("ORDER", "Order Creation", createOrderDTO.getId().toString(),
                    "CPF %s is fraud suspect".formatted(createOrderDTO.getCpf()));
            return new ServiceResponse("FRAUD_SUSPECT", "CPF %s is fraud suspect".formatted(createOrderDTO.getCpf()));
        }
